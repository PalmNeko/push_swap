...
int	ps_sort_for_three(t_push_swap *ps)
{
	...
	if (ft_lstsize(ps->stack_a) != 3)
		return (0);
	first = *(int *)ft_lstat(ps->stack_a, 0)->content;
	second = *(int *)ft_lstat(ps->stack_a, 1)->content;
	third = *(int *)ft_lstat(ps->stack_a, 2)->content;
	max = ft_max(first, ft_max(second, third));
	if (max == first && ps_rra(ps) == -1)
		return (-1);
	if (max == second && ps_ra(ps) == -1)
		return (-1);
	first = *(int *)ft_lstat(ps->stack_a, 0)->content;
	second = *(int *)ft_lstat(ps->stack_a, 1)->content;
	third = *(int *)ft_lstat(ps->stack_a, 2)->content;
